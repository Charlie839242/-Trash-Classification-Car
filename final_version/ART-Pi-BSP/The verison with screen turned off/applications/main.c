...
int main(void)
{
    ...
    uint32_t total_waitTime = 0;                    // 用来统计时间

    int number = 0;                                 // nms筛选出来的box的个数
    ...
    yolo_region_layer_init(&r1, 0.95, 125, 0, 5);    // init nms rules

    ai_camera();

#if debug_flag
    waitTime = rt_tick_get() - startTs;
    printf("The cost time for the initialization is %ld\n", waitTime);
#endif
...
//                        RT_NULL);
...
#if debug_flag
        waitTime = rt_tick_get() - startTs;
        printf("The cost time for the function 'rt_event_recv' is %ld\n", waitTime);
#endif
...
#if debug_flag
        waitTime = rt_tick_get() - startTs;
        printf("The cost time for the function 'lcd->parent.control' is %ld\n", waitTime);
#endif
        ...
        if(ai_flag > 0)
        {
...
#if debug_flag
            waitTime = rt_tick_get() - startTs;
            printf("The cost time for the function 'rgb2gray' is %ld\n", waitTime);
#endif
...
#if debug_flag
            waitTime = rt_tick_get() - startTs;
            printf("The cost time for the function 'bilinera' is %ld\n", waitTime);
#endif
...
#if debug_flag
            waitTime = rt_tick_get() - startTs;
            printf("The cost time for the function '_itof' is %ld\n", waitTime);
#endif
...
#if debug_flag
            waitTime = rt_tick_get() - startTs;
            printf("The cost time for the function 'yolo_decode' is %ld\n", waitTime);
#endif
            //////////////////////////////////////////////////////////
            ...
            for(int i=0;i<125;i++)
            {
                ...
                float p2 = p->class_score2 * p->objectness;
                s[i].obj = (p0>p1) ? ((p2>p0)?p2:p0):((p2>p1)?p2:p1);
                s[i].class = (p0>p1) ?((p2>p0)?3:0):((p2>p1)?3:2);
                s[i].key = i;
            }

            qsort(s, 125, sizeof(obj_key), nms_comparator);     // 按照obj_key.obj来排序
...
//                rt_kprintf("detected !\n");
...
#if debug_flag
            waitTime = rt_tick_get() - startTs;
            printf("The cost time for the function 'nms' is %ld\n", waitTime);
#endif
            /////////////////////////////////////////////////////////
            ...
            for(int t=0; t<number; t++)
            {
                if(s[t].obj > r1.threshold)
                {
                     ...
                     sign = s[t].class;
                     sprintf(str_tem,"sign=%d\n",sign);
                     rt_kprintf(str_tem);



                     if(sign == 0){
                         rt_kprintf("class = 0 ! It is a bottle!\n");}
                     else if(sign == 3){
                         rt_kprintf("class = 3 ! It is a tri!\n");}
                     else if(sign ==2){
                         rt_kprintf("class = 2 ! It is a box!\n");}
#if see_box
...
#if debug_flag
            waitTime = rt_tick_get() - startTs;
            printf("The cost time for the function 'show_boxes' is %ld\n", waitTime);
#endif
            //////////////////////////////////////////////////////////
#if debug_flag
            startTs = rt_tick_get();
...
#if debug_flag
            waitTime = rt_tick_get() - startTs;
            printf("The cost time for the function 'DCMI_Start' is %ld\n", waitTime);
#endif
        }

        total_waitTime = rt_tick_get() - total_start;
        printf("The cost time for one frame is %ld\n", total_waitTime);
    }
