...
{
  {14,44}, {20,73}, {55,38}, {27,104}, {100,84}
};
...
int yolo_decode(float *out_data)
{
  ...
  for(int i=0; i<5*5*5; i++)
  {
    ...
    float x_tmp = 1 / (1 + exp(-out_data[i*8+0])); // sigmoid decode
    float y_tmp = 1 / (1 + exp(-out_data[i*8+1]));
    float box_x = (x_tmp + k) / 5;                 //
    float box_y = (y_tmp + l) / 5;
    
    float box_w = (exp(out_data[i*8+2])*anchor[j][0])/ input_dims[0];
    float box_h = (exp(out_data[i*8+3])*anchor[j][1])/ input_dims[1];
    
    float objectness = 1 / (1 + exp(-out_data[i*8+4]));
    
    float class_scores0 = 1 / (1 + exp(-out_data[i*8+5]));
    float class_scores1 = 1 / (1 + exp(-out_data[i*8+6]));
    float class_scores2 = 1 / (1 + exp(-out_data[i*8+7]));
    
    out_data[i*8+0] = box_x;
    out_data[i*8+1] = box_y;
    out_data[i*8+2] = box_w;
    out_data[i*8+3] = box_h;
    out_data[i*8+4] = objectness;
    out_data[i*8+5] = class_scores0;
    out_data[i*8+6] = class_scores1;
    out_data[i*8+7] = class_scores2;
